...
if (style == FALSE){
  method <- usethis::ui_yeah("Do you want to use the centroid approach?")

  if (method == FALSE){
    intersect <- usethis::ui_yeah("OK! We'll use the intersect approach instead. Is that OK?")

    if (intersect == FALSE){
      stop("Workflow aborted!")
    }
  }
}
...
if (style == FALSE){
  ...
  create_year <- function(year, intersect_by, method = "intersects"){

    ## download data
    zctas <- zctas(year = year, class = "sf")
    zctas <- st_transform(zctas, crs = 3857)

    ## subset
    if (year >= 2020){
      zctas <- select(zctas, GEOID = GEOID20)
    } else if (year >= 2010 & year < 2020){
      zctas <- select(zctas, GEOID = GEOID10)
    }
    ...
    ## geoprocess
    if (method == "centroid"){
      zctas <- st_centroid(zctas)
    }

    intersect_df <- st_intersection(zctas, intersect_by)
...
if (style == FALSE){
  ...
  if (method == FALSE){
    zcta2021 <- create_year(year = 2021, intersect_by = states_abbrev)
    save(zcta2021, file = "data-raw/zcta2021.rda")

    zcta2020 <- create_year(year = 2020, intersect_by = states_abbrev)
    save(zcta2020, file = "data-raw/zcta2020.rda")

    zcta2019 <- create_year(year = 2019, intersect_by = states_abbrev)
    save(zcta2019, file = "data-raw/zcta2019.rda")

    zcta2018 <- create_year(year = 2018, intersect_by = states_abbrev)
    save(zcta2018, file = "data-raw/zcta2018.rda")

    zcta2017 <- create_year(year = 2017, intersect_by = states_abbrev)
    save(zcta2017, file = "data-raw/zcta2017.rda")

    zcta2016 <- create_year(year = 2016, intersect_by = states_abbrev)
    save(zcta2016, file = "data-raw/zcta2016.rda")

    zcta2015 <- create_year(year = 2015, intersect_by = states_abbrev)
    save(zcta2015, file = "data-raw/zcta2015.rda")

    zcta2014 <- create_year(year = 2014, intersect_by = states_abbrev)
    save(zcta2014, file = "data-raw/zcta2014.rda")

    zcta2013 <- create_year(year = 2013, intersect_by = states_abbrev)
    save(zcta2013, file = "data-raw/zcta2013.rda")

    zcta2012 <- create_year(year = 2012, intersect_by = states_abbrev)
    save(zcta2012, file = "data-raw/zcta2012.rda")

    # doesn't work? not currently on tigris
    # zcta2011 <- create_year(year = 2011, intersect_by = states_abbrev)
    # save(zcta2011, file = "data-raw/zcta2011.rda")

    zcta2010 <- create_year(year = 2010, intersect_by = states_abbrev)
    save(zcta2010, file = "data-raw/zcta2010.rda")

  } else if (method == TRUE){

    zcta2021 <- create_year(year = 2021, intersect_by = states_abbrev, method = "centroid")
    save(zcta2021, file = "data-raw/zcta2021_centroid.rda")

    zcta2020 <- create_year(year = 2020, intersect_by = states_abbrev, method = "centroid")
    save(zcta2020, file = "data-raw/zcta2020_centroid.rda")

    zcta2019 <- create_year(year = 2019, intersect_by = states_abbrev, method = "centroid")
    save(zcta2019, file = "data-raw/zcta2019_centroid.rda")

    zcta2018 <- create_year(year = 2018, intersect_by = states_abbrev, method = "centroid")
    save(zcta2018, file = "data-raw/zcta2018_centroid.rda")

    zcta2017 <- create_year(year = 2017, intersect_by = states_abbrev, method = "centroid")
    save(zcta2017, file = "data-raw/zcta2017_centroid.rda")

    zcta2016 <- create_year(year = 2016, intersect_by = states_abbrev, method = "centroid")
    save(zcta2016, file = "data-raw/zcta2016_centroid.rda")

    zcta2015 <- create_year(year = 2015, intersect_by = states_abbrev, method = "centroid")
    save(zcta2015, file = "data-raw/zcta2015_centroid.rda")

    zcta2014 <- create_year(year = 2014, intersect_by = states_abbrev, method = "centroid")
    save(zcta2014, file = "data-raw/zcta2014_centroid.rda")

    zcta2013 <- create_year(year = 2013, intersect_by = states_abbrev, method = "centroid")
    save(zcta2013, file = "data-raw/zcta2013_centroid.rda")

    zcta2012 <- create_year(year = 2012, intersect_by = states_abbrev, method = "centroid")
    save(zcta2012, file = "data-raw/zcta2012_centroid.rda")

    zcta2010 <- create_year(year = 2010, intersect_by = states_abbrev, method = "centroid")
...
rm(zcta2010, zcta2012, zcta2013, zcta2014, zcta2015, zcta2016, zcta2017,
   zcta2018, zcta2019, zcta2020, zcta2021)
...
reference <- data.frame(
  state = sort(rep(states_abbrev$STUSPS, length(2010:2021))),
  year = rep(2010:2021, length(states_abbrev$STUSPS))
)
